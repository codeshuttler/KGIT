...
def test(input_path: str, output_path: str, model, device, use_context=True):
    ...
    for i, batch in enumerate(tqdm.tqdm(chunked(dataset, chunk_size), total=len(dataset)//chunk_size)):
        ...
        if i % 5000 == 0 and batch_end_time - batch_start_time > 15:
...
if __name__ == "__main__":
    ...
    elif args.model == "rag-token-nq":
        model = RAG("./models/rag-token-nq", device=args.device)
    elif args.model == "albert":
        model = Albert(device=args.device)
    else:
