...
def venn2_ax(ax, labels, names=['A', 'B'], **options):
    """
    plots a 2-set Venn diagram
    @type labels: dict[str, str]
    @type names: list[str]
    ...
    draw_text(ax, 0.20, 0.56, names[0], colors[0], fontsize=fontsize, ha="right", va="bottom")
    draw_text(ax, 0.80, 0.56, names[1], colors[1], fontsize=fontsize, ha="left", va="bottom")
    # legend
    if legend:
      leg = ax.legend(names, loc='center left', bbox_to_anchor=(1.0, 0.5), fancybox=True)
      leg.get_frame().set_alpha(0.5)
...
def venn3_ax(ax, labels, names=['A', 'B', 'C'], **options):
    """
    plots a 3-set Venn diagram
    @type labels: dict[str, str]
    @type names: list[str]
    ...
    # legend
    draw_text(ax, 0.15, 0.87, names[0], colors[0], fontsize=fontsize, ha="right", va="bottom")
    draw_text(ax, 0.85, 0.87, names[1], colors[1], fontsize=fontsize, ha="left", va="bottom")
    draw_text(ax, 0.50, 0.02, names[2], colors[2], fontsize=fontsize, va="top")
    if legend:
      leg = ax.legend(names, loc='center left', bbox_to_anchor=(1.0, 0.5), fancybox=True)
      leg.get_frame().set_alpha(0.5)
