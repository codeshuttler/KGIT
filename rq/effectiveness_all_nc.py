...
def main():
    ...
    for model_i, model in enumerate(MODELS):
        ...
        for kr_i, kr in enumerate(KRS):
            print(model, kr)
            check_file = f"result/{kr}/check_nc_all_{model}.txt"
            if not os.path.exists(check_file):
                dataframe_data[f"ir{kr_i + 1}_bug"].append(0)
                dataframe_data[f"ir{kr_i + 1}_total"].append(0)
                continue
            with open(check_file, 'r', encoding='utf-8') as f:
                lines = f.readlines()
            dataframe_data[f"ir{kr_i + 1}_bug"].append(len(lines))

            answer_file = f"result/{kr}/answers_nc_{model}.txt"
            ...
            for answer in answers:
                ...
                if is_all_pass(preliminaries):
                    pre_pass_count += 1
            dataframe_data[f"ir{kr_i + 1}_total"].append(pre_pass_count)
    
    df = pd.DataFrame(dataframe_data)
    df.to_excel("rq_result/effectiveness_all_nc.xlsx")
